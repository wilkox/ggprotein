...
#' Convert alongness to millimetres, in the current viewport
#'
#' @noRd
alongness_to_mm <- function(alongness, coord_system, r = NULL) {

  if (coord_system == "cartesian") {
    return(grid::convertWidth(
      grid::unit(alongness, "native"),
      "mm",
      valueOnly = TRUE
    ))
    ...
    circumference <- 2 * pi * 
      convertWidth(grid::unit(r, "native"), "mm", valueOnly = TRUE)
    return(alongness / r * circumference)

  } else if (coord_system == "flip") {
    return(grid::convertHeight(
      grid::unit(alongness, "native"),
      "mm",
      valueOnly = TRUE
    ))
